...
def _get_file_paths_and_labels(data_path: Path, subset_size: Union[int, None]) -> Tuple[List[str], List[int]]:
    ...
    if subset_size: print(f"Modo Debug: A usar um subconjunto de {subset_size} imagens no total.")

    for class_dir in data_path.iterdir():
        if not class_dir.is_dir() or class_dir.name not in class_name_to_id: continue
        class_name = class_dir.name
        label_id = class_name_to_id[class_name]
        for image_file in sorted(class_dir.glob('*.*')):
            image_paths.append(str(image_file)); image_labels.append(label_id)
...
def create_acgan_dataset(data_path: Path, batch_size: int) -> Tuple[Dataset, int]:
    ...
    if not paths: raise ValueError(f"Nenhuma imagem encontrada em {data_path}.")
    ...
    def _parse_and_process_image(path, label):
        ...
        if config.Model.COLOR_MODE != 'auto':
            ...
            original_channels = tf.cond(
                tf.equal(tf.rank(img_decoded_for_shape), 3),
                lambda: shape_tensor[2],
                lambda: tf.constant(1, dtype=tf.int32)
            )

            target_channels = tf.constant(config.Model.CHANNELS, dtype=tf.int32)
            if tf.not_equal(original_channels, target_channels):
                tf.py_function(_print_warning, inp=[path, original_channels, target_channels], Tout=tf.bool)
        
        img = tf.image.decode_image(img_raw, channels=config.Model.CHANNELS, expand_animations=False)
        ...
        if isinstance(config.Model.IMG_HEIGHT, int) and isinstance(config.Model.IMG_WIDTH, int):
            ...
            original_height, original_width = tf.cast(original_shape[0], tf.float32), tf.cast(original_shape[1], tf.float32)
            if isinstance(config.Model.IMG_HEIGHT, int):
                new_height = tf.constant(config.Model.IMG_HEIGHT, dtype=tf.float32)
                new_width = tf.cast(new_height * (original_width / original_height), tf.int32)
                ...
                new_height = tf.cast(new_width * (original_height / original_width), tf.int32)
    ...
    dataset = (
        path_ds.shuffle(buffer_size=num_images)
        .map(_parse_and_process_image, num_parallel_calls=tf.data.AUTOTUNE)
        .cache().batch(batch_size).prefetch(buffer_size=tf.data.AUTOTUNE)
    )
